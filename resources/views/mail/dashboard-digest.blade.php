        ...
        <tbody>
            ...
            @foreach ($metricDefinitions as $definition)
                ...
                @foreach ($metricItems as $item)
                    ...
                    @endphp
                    <tr>
                        <td style="{{ $listLeft.$stripe }}">{{ $item['label'] }}</td>
                        ...
                        @foreach ($metricDefinitions as $column)
                            ...
                            @if ($column['key'] === $definition['key'])
                                <td style="{{ $listCell.$stripe }}">{{ number_format($item['count'] ?? 0) }}</td>
                                <td style="{{ $listMuted.$stripe }}">{{ $item['percent'] === null ? '—' : number_format($item['percent'], 1).'%' }}</td>
                            @else
                                <td style="{{ $listCell.$stripe }}"></td>
                    ...
                    @foreach ($item['items'] ?? [] as $nested)
                        <tr>
                            <td style="{{ $listLeft.$stripe }} padding-left: 32px;">{{ $nested['label'] }}</td>
                            <td style="{{ $listCell.$stripe }}"></td>
                            @foreach ($metricDefinitions as $column)
                                @continue($column['key'] === 'total_leads_called')
                                @if ($column['key'] === $definition['key'])
                                    <td style="{{ $listCell.$stripe }}">{{ number_format($nested['count'] ?? 0) }}</td>
                                    <td style="{{ $listMuted.$stripe }}">{{ $nested['percent'] === null ? '—' : number_format($nested['percent'], 1).'%' }}</td>
                                @else
                                    <td style="{{ $listCell.$stripe }}"></td>
                                    <td style="{{ $listMuted.$stripe }}"></td>
                                @endif
