...
echo '==== ASSIGNED AT BACKFILL ===='
docker compose exec -T db psql -U onpoint -d onpoint_call -c "
SELECT
  COUNT(*) FILTER (WHERE calling_list_id IS NOT NULL) AS on_list,
  COUNT(*) FILTER (WHERE calling_list_id IS NOT NULL AND calling_list_assigned_at IS NOT NULL) AS stamped,
  COUNT(*) FILTER (WHERE calling_list_id IS NOT NULL AND calling_list_assigned_at IS NULL) AS unstamped
FROM leads;
"
...
curl -fsS -o /dev/null -w 'lists:%{http_code}\n' http://localhost/admin/calling-lists
